...
namespace
{
  constexpr wchar_t kIniFileName[] = L"hakorc.ini";
  constexpr wchar_t kIniSection[] = L"hakoWinAppsRC";
...
BOOL ChakoWinAppsDlg::OnInitDialog()
{
  ...
  // HakoWinPathの読み込み
  GetPrivateProfileString(_T("hakoWinAppsRC"),
    _T("HakoWinPath"),
    _T("C:\\Initial\\Path"),
    szPath,
    MAX_PATH,
  ...
  int fileSizeMB = GetPrivateProfileInt(_T("hakoWinAppsRC"),
    _T("ZeroFileSizeMB"),
    5, // デフォルト値
    strFilePath);
...
// RCファイルを取得
  CString currentPath = GetModuleDirectory();
  CString searchPath = m_HakoRCConf;  // RC Configの検索バスの設定
#ifdef _DEBUG
  AfxMessageBox(currentPath);
  AfxMessageBox(searchPath);
#endif
  hakoWinRCDialog dlg(currentPath, searchPath, this);

  if (dlg.DoModal() == IDOK)
  {
    CString chosen = dlg.m_HakoRCConf;
    if (!chosen.IsEmpty())
    {
     ...
     // ゲームコントローラの判別
     std::string pycommand = "python ./controller_check.py";
     ...
     if(result != 0){
       AfxMessageBox(_T("ゲームコントローラの認識に失敗しました"));
	     return;     
     }
     
     directory = std::string(CT2A(m_HakoWinPath)) + "\\drone_api";
     pycommand = "python -m rc-custom " +
       std::string(CT2A(m_HakoRCAvatar)) +
       " ./rc/rc_config/" + std::string(CT2A(chosen));
     command = "try { " + pycommand + " } catch { Write-Host 'Error:' $_; pause }";

     runPowerShellCommand(command, directory);
#ifdef _DEBUG
      CString fullPath = searchPath + _T("\\") + chosen;
      AfxMessageBox(_T("選択されたファイル: ") + fullPath);
#endif
    }
  }
...
void ChakoWinAppsDlg::OnBnClickedButton3()
{
  ...
  // PowerShellコマンドを構築
  pycommand = "python -m replay.hako_asset_replayer --replay ./config/replay/replay.json";
  command = "try { " + pycommand + " } catch { Write-Host 'Error:' $_; pause }";
  // PowerShellコマンドを実行
  runPowerShellCommand(command, directory);
